...
function renderRooms(){ const arr=JSON.parse(localStorage.getItem(roomsKey)||"[]"); const box=$("roomsList"); box.innerHTML=""; arr.forEach(r=>{let b=document.createElement("button"); b.textContent=r; b.dataset.room=r; if(r===room){ b.classList.add("activeRoom"); b.setAttribute("aria-current","true"); } else { b.classList.remove("activeRoom"); b.removeAttribute("aria-current"); } b.onclick=()=>join(r); box.appendChild(b);}); }
...
// === Telegram button handler (safe CSP-compatible) ===
document.addEventListener("DOMContentLoaded", () => {
  const tgBtn = document.querySelector('[title="Telegram"]');
  if (tgBtn) {
    tgBtn.addEventListener("click", () => {
      window.open("https://t.me/anonghostchat", "_blank", "noopener,noreferrer");
    });
  }
});
